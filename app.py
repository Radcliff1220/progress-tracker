...
def seed_defaults(conn: sqlite3.Connection) -> None:
    ...
    old_project_count = conn.execute(
        "SELECT COUNT(*) FROM projects WHERE name IN ('A', 'B', 'C')"
    ).fetchone()[0]

    if submission_count == 0 and (user_count == 0 or old_project_count > 0):
        conn.executescript(
            """
...
def get_stats() -> dict:
    with connect() as conn:
        ...
        research_latest = rows(
            conn.execute(
                """
                SELECT u.id AS user_id, u.name AS user_name, rs.title, rs.stage, rs.progress, rs.created_at
        ...
        research_history = rows(
            conn.execute(
                """
                SELECT rs.id, u.id AS user_id, u.name AS user_name, rs.title, rs.stage, rs.progress, rs.created_at
                FROM research_submissions rs
                JOIN users u ON u.id = rs.user_id
