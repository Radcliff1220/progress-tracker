...
CATALOG_VERSION = "2026-07-08-catalog-v3"
...
def migrate_db(conn: sqlite3.Connection) -> None:
    research_columns = {
        row["name"]
        for row in conn.execute("PRAGMA table_info(research_submissions)").fetchall()
    }
    if "note" not in research_columns:
        conn.execute("ALTER TABLE research_submissions ADD COLUMN note TEXT NOT NULL DEFAULT ''")
    ...
    for project in DEFAULT_PROJECTS:
        for task in DEFAULT_TASKS:
            conn.execute(
                ...
                VALUES(?, ?, 10, 0, 1, ?)
                """,
                (project_ids[project], task, now_text()),
            )
...
def get_stats() -> dict:
    with connect() as conn:
        ...
        contributions = rows(
            conn.execute(
                ...
                SELECT
                    ...
                    p.id AS project_id,
                    p.name AS project_name,
                    ROUND(SUM(t.workload * latest.progress / 100.0), 2) AS contribution_workload,
                    ROUND(
                        CASE WHEN totals.total_workload = 0 THEN 0
                        ELSE SUM(t.workload * latest.progress / 100.0) * 100.0 / totals.total_workload
                        END,
                        2
        ...
        project_latest_submissions = rows(
            conn.execute(
                ...
                SELECT latest.id, u.id AS user_id, u.name AS user_name, p.id AS project_id,
                       ...
                       t.workload, ROUND(t.workload * latest.progress / 100.0, 2) AS contribution_workload,
                       latest.progress, latest.note, latest.created_at
                FROM latest
                JOIN users u ON u.id = latest.user_id
                JOIN projects p ON p.id = latest.project_id
                JOIN tasks t ON t.id = latest.task_id
        ...
        research_latest = rows(
            conn.execute(
                """
                SELECT u.id AS user_id, u.name AS user_name, rs.title, rs.stage, rs.progress, rs.note, rs.created_at
        ...
        research_history = rows(
            conn.execute(
                """
                SELECT rs.id, u.id AS user_id, u.name AS user_name, rs.title, rs.stage, rs.progress, rs.note, rs.created_at
                FROM research_submissions rs
                JOIN users u ON u.id = rs.user_id
