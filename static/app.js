...
const RESEARCH_WEEK_ORDER = [
  "胡添欢", "蒋竺宴", "高兴", "乔舒", "房孟君", "焉城菡", "秦钰叶", "余欣然",
  "张明威", "张润莹", "徐浩阳", "陈海龙", "高林越", "吕志锦", "崔永平", "朱浩楠",
  "王彩霞", "童一凡", "叶栩彤", "李鹏飞", "邵梓效", "陈宜建", "刘自豪", "张骁",
  "胡恺元", "李浩月", "毛文涛", "刘倩竹", "刘康", "孙榕", "蔡年杰", "黄梦娇",
];
const RESEARCH_WEEK_ORDER_INDEX = new Map(RESEARCH_WEEK_ORDER.map((name, index) => [name, index]));
...
let state = {
  ...
  adminSection: "projectAdminSection",
  researchUserFilter: "all",
  researchTimeFilter: "all",
  adminPassword: localStorage.getItem("adminPassword") || "",
};

const $ = (id) => document.getElementById(id);
const pct = (value) => `${Number(value || 0).toFixed(1).replace(".0", "")}%`;
const statusLabel = (status) => status === "revoked" ? "已撤回" : "有效";
...
function reviewControls(type, row) {
  return `
    <div class="review-controls">
      <select data-review-status="${type}-${row.id}">
        <option value="active" ${row.status !== "revoked" ? "selected" : ""}>有效</option>
        <option value="revoked" ${row.status === "revoked" ? "selected" : ""}>撤回</option>
      </select>
      <textarea data-review-reply="${type}-${row.id}" placeholder="管理员回复意见">${escapeHtml(row.admin_reply || "")}</textarea>
      <button data-review-type="${type}" data-review-id="${row.id}">保存回复</button>
    </div>
  `;
}

function submissionStatusCells(row) {
  return `
    <td>${statusLabel(row.status)}</td>
    <td>${escapeHtml(row.admin_reply || "")}</td>
    <td>${row.admin_replied_at || ""}</td>
  `;
}
...
function confirmProjectSubmission(payload) {
  const user = state.users.find((item) => item.id === payload.user_id)?.name || "";
  const project = state.projects.find((item) => item.id === payload.project_id)?.name || "";
  const task = state.tasks.find((item) => item.id === payload.task_id)?.name || "";
  return window.confirm(
    `请确认项目进展填报：\n\n姓名：${user}\n项目：${project}\n任务：${task}\n进度：${payload.progress}%\n细节汇报：${payload.note}\n\n确认提交？`
  );
}

function confirmResearchSubmission(payload) {
  const user = state.users.find((item) => item.id === payload.user_id)?.name || "";
  return window.confirm(
    `请确认科研进展填报：\n\n姓名：${user}\n大论文标题：${payload.title}\n阶段：${payload.stage}\n进度：${payload.progress}%\n细节汇报：${payload.note}\n\n确认提交？`
  );
...
function setSelectValue(id, value) {
  const node = $(id);
  if (node && [...node.options].some((option) => option.value === String(value))) {
    node.value = String(value);
  }
}

function inTimeFilter(createdAt, filter) {
  if (filter === "all") return true;
  const time = new Date(String(createdAt || "").replace(" ", "T"));
  if (Number.isNaN(time.getTime())) return true;
  const days = filter === "week" ? 7 : 30;
  return Date.now() - time.getTime() <= days * 24 * 60 * 60 * 1000;
}

function applyResearchFilters(rows) {
  return (rows || []).filter((row) => {
    const userOk = state.researchUserFilter === "all" || String(row.user_id) === String(state.researchUserFilter);
    return userOk && inTimeFilter(row.created_at, state.researchTimeFilter);
  });
}

function sortResearchRows(rows) {
  const copy = [...(rows || [])];
  if (state.researchTimeFilter !== "week") {
    return copy.sort((a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")));
  }
  return copy.sort((a, b) => {
    const ai = RESEARCH_WEEK_ORDER_INDEX.has(a.user_name) ? RESEARCH_WEEK_ORDER_INDEX.get(a.user_name) : 9999;
    const bi = RESEARCH_WEEK_ORDER_INDEX.has(b.user_name) ? RESEARCH_WEEK_ORDER_INDEX.get(b.user_name) : 9999;
    if (ai !== bi) return ai - bi;
    return String(b.created_at || "").localeCompare(String(a.created_at || ""));
  });
}

function renderResearchMissingHint() {
  const hint = $("researchMissingHint");
  if (!hint) return;
  if (state.researchTimeFilter !== "week") {
  ...
  const filled = new Set(
    (state.stats.research_history || [])
      .filter((row) => row.status !== "revoked" && inTimeFilter(row.created_at, "week"))
      .map((row) => row.user_name)
      .filter(Boolean)
  );
  const missing = RESEARCH_WEEK_ORDER.filter((name) => !filled.has(name));
  hint.classList.remove("hidden");
  hint.innerHTML = missing.length
    ? `<strong>近一周未填报：</strong>${missing.map(escapeHtml).join("、")}`
    : `<strong>近一周未填报：</strong>无，名单内人员均已填报。`;
}
...
function renderMyHistory() {
  ...
  table($("myProjectHistory"), ["时间", "项目", "任务", "进度", "贡献工作量", "细节汇报", "状态", "管理员回复", "回复时间"], projectRows, (row) => `
    <tr>
      <td>${row.created_at}</td>
      ...
      ${submissionStatusCells(row)}
    </tr>
  `);
  table($("myResearchHistory"), ["时间", "标题", "阶段", "进度", "细节汇报", "状态", "管理员回复", "回复时间"], researchRows, (row) => `
    <tr><td>${row.created_at}</td><td>${escapeHtml(row.title)}</td><td>${escapeHtml(row.stage)}</td><td>${pct(row.progress)}</td><td>${escapeHtml(row.note)}</td>${submissionStatusCells(row)}</tr>
  `);
}
...
function renderAdminSections() {
  ...
  document.querySelectorAll(".admin-section").forEach((section) => {
    section.classList.toggle("active", section.id === state.adminSection);
  });
}

function renderResearchFilters() {
  const userFilter = $("researchUserFilter");
  if (!userFilter) return;
  userFilter.innerHTML = `<option value="all">全部人员</option>` + state.users
    .filter((user) => user.active !== 0)
    .map((user) => `<option value="${user.id}">${escapeHtml(user.name)}</option>`)
    .join("");
  setSelectValue("researchUserFilter", state.researchUserFilter);
  setSelectValue("researchTimeFilter", state.researchTimeFilter);
}
...
function renderProjectDetail() {
  ...
  $("projectDetail").innerHTML = `
    ...
    <div class="grid two">
      <section>
        <h3>个人项目贡献明细</h3>
        <div class="table-wrap tall-table"><table id="projectContributionDetail"></table></div>
      </section>
      <section>
        <h3>任务修改与确认</h3>
    ...
    <div class="grid two">
      <section>
        <h3>项目推进历史</h3>
        <div class="table-wrap tall-table"><table id="projectHistoryDetail"></table></div>
      </section>
      <section>
        <h3>确认修正历史</h3>
  ...
  table($("projectContributionDetail"), ["姓名", "任务", "最新填报进度", "工作量", "贡献工作量", "时间", "细节汇报", "状态", "管理员回复", "回复时间", "操作"], latestSubmissions, (row) => `
    <tr>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.task_name)}</td>
      <td>${pct(row.progress)}</td>
      ...
      <td>${escapeHtml(row.note)}</td>
      ${submissionStatusCells(row)}
      <td>${reviewControls("project", row)}</td>
    </tr>
  `);
  ...
  table($("projectHistoryDetail"), ["时间", "姓名", "任务", "进度", "细节汇报", "状态", "管理员回复", "回复时间", "操作"], history, (row) => `
    <tr>
      <td>${row.created_at}</td>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.task_name)}</td>
...
function renderResearchStats() {
  const latestRows = sortResearchRows(applyResearchFilters(state.stats.research_latest));
  const historyRows = sortResearchRows(applyResearchFilters(state.stats.research_history));
  renderResearchMissingHint();
  table($("researchStats"), ["姓名", "大论文标题", "阶段", "进度", "细节汇报", "状态", "管理员回复", "回复时间", "更新时间", "操作"], latestRows, (row) => `
    <tr>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.title)}</td>
      <td>${escapeHtml(row.stage)}</td>
      ...
      ${submissionStatusCells(row)}
      <td>${row.created_at}</td>
      <td>${reviewControls("research", row)}</td>
    </tr>
  `);
  table($("researchHistoryAdmin"), ["时间", "姓名", "大论文标题", "阶段", "进度", "细节汇报", "状态", "管理员回复", "回复时间", "操作"], historyRows, (row) => `
    <tr>
      <td>${row.created_at}</td>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.title)}</td>
...
document.addEventListener("click", async (event) => {
  try {
    ...
    const adminTab = event.target.closest("[data-admin-section]");
    if (adminTab) {
      state.adminSection = adminTab.dataset.adminSection;
      renderAdminSections();
    }
    ...
    if (reviewId) {
      const type = event.target.dataset.reviewType;
      const key = `${type}-${reviewId}`;
      await api("/api/admin/review-submission", {
        method: "POST",
        body: JSON.stringify({
          type,
          id: Number(reviewId),
          status: document.querySelector(`[data-review-status="${key}"]`).value,
          admin_reply: document.querySelector(`[data-review-reply="${key}"]`).value,
        }),
      });
      toast("审核意见已保存");
      await loadAll();
...
$("researchUserFilter").addEventListener("change", () => {
  state.researchUserFilter = $("researchUserFilter").value;
  renderResearchStats();
});
$("researchTimeFilter").addEventListener("change", () => {
  state.researchTimeFilter = $("researchTimeFilter").value;
  renderResearchStats();
});
...
$("projectForm").addEventListener("submit", async (event) => {
  ...
  try {
    const note = requireValue("projectNote", "项目细节汇报");
    if (note === null) return;
    const payload = {
    ...
    if (!payload.user_id || !payload.project_id || !payload.task_id || !payload.progress) {
      toast("请完整选择姓名、项目、任务和进度");
      return;
    }
    if (!confirmProjectSubmission(payload)) return;
...
$("researchForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  try {
    const title = requireValue("researchTitle", "大论文标题");
    if (title === null) return;
    const note = requireValue("researchNote", "科研细节汇报");
    if (note === null) return;
    const payload = {
      user_id: currentUserId(),
      title,
    ...
    if (!payload.user_id || !payload.stage || !payload.progress) {
      toast("请完整选择姓名、阶段和进度");
      return;
    }
    if (!confirmResearchSubmission(payload)) return;
