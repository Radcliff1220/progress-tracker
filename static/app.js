...
function renderMyHistory() {
  ...
  table($("myProjectHistory"), ["时间", "项目", "任务", "进度", "贡献工作量", "备注"], projectRows, (row) => `
    <tr>
      <td>${row.created_at}</td>
  ...
  `);
  table($("myResearchHistory"), ["时间", "标题", "阶段", "进度", "备注"], researchRows, (row) => `
...
function renderProjectDetail() {
  ...
  table($("projectContributionDetail"), ["姓名", "任务", "最新填报进度", "工作量", "贡献工作量", "时间", "备注"], latestSubmissions, (row) => `
    <tr>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.task_name)}</td>
      <td>${pct(row.progress)}</td>
  ...
  table($("projectHistoryDetail"), ["时间", "姓名", "任务", "进度", "备注"], history, (row) => `
    <tr>
      <td>${row.created_at}</td>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.task_name)}</td>
  ...
  table($("projectConfirmHistory"), ["时间", "任务", "确认进度", "备注"], confirmations, (row) => `
    <tr>
      <td>${row.created_at}</td>
      <td>${escapeHtml(row.task_name)}</td>
...
function renderResearchStats() {
  table($("researchStats"), ["姓名", "大论文标题", "阶段", "进度", "备注", "更新时间"], state.stats.research_latest || [], (row) => `
    <tr>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.title)}</td>
      <td>${escapeHtml(row.stage)}</td>
