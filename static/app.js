...
function renderMyHistory() {
  ...
  table($("myResearchHistory"), ["时间", "标题", "阶段", "进度"], researchRows, (row) => `
    <tr><td>${row.created_at}</td><td>${escapeHtml(row.title)}</td><td>${escapeHtml(row.stage)}</td><td>${pct(row.progress)}</td></tr>
  `);
}
...
function renderProjectDetail() {
  ...
  table($("projectContributionDetail"), ["姓名", "任务", "填报进度", "工作量", "贡献工作量", "时间", "备注"], history, (row) => `
    <tr>
      <td>${escapeHtml(row.user_name)}</td>
      <td>${escapeHtml(row.task_name)}</td>
      <td>${pct(row.progress)}</td>
